...
public class TimexSETAnnotator extends CleartkSequenceAnnotator<String>{

	private Logger logger = Logger.getLogger(TimexSETAnnotator.class);
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
			...
			for (Sentence sentence : JCasUtil.selectCovered(jCas, Sentence.class, section)) {
				...
				if (this.isTraining()) {
					...
					for(Timex3 date : times){
						if(date.getTimex3Type().equals("SET")){
		...
		for (Timex3 time : JCasUtil.select(jCas, Timex3.class)) {
			if (time.getTimex3Type() == null) {
				time.setId("TSET"+timeIndex);
				time.setTimex3Type("SET");
