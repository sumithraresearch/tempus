...
public class TimexQUANTIFIERAnnotator extends CleartkSequenceAnnotator<String>{

	private Logger logger = Logger.getLogger(TimexQUANTIFIERAnnotator.class);
...
//	private FeatureExtractor1<Event> added_event_features;
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
		logger.info("Processing QUANTIFIERs");
...
/*					Map<BaseToken,Collection<de.unihd.dbs.uima.types.heideltime.Timex3>> coveringSection = JCasUtil.indexCovering(jCas,
				...
				if (this.isTraining()) {
					...
					for(Timex3 date : times){
						if(date.getTimex3Type().equals("QUANTIFIER")){							
							dates.add(date);
						}
					}
		...
		for (Timex3 time : JCasUtil.select(jCas, Timex3.class)) {
			if(time.getTimex3Type()==null){
				time.setTimex3Type("QUANTIFIER");
				time.setId("TQUANTIFIER"+timeIndex);
			}
			timeIndex += 1;
		}
	}
	public static AnalysisEngineDescription getClassifierDescription(String modelFileName)
		...
		return AnalysisEngineFactory.createEngineDescription(
				TimexQUANTIFIERAnnotator.class,
